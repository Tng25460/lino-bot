...
backup_file() {
  local f="$1"
  [[ -f "$f" ]] || return 0
  local b="${f}.bak_$(stamp)"
  cp -a "$f" "$b"
  echo "   backup: $f -> $b"
}

echo
echo "==> 1) Create modules"
mkdir -p src core

cat > src/vnext_guard.py <<'PY'
...
echo
echo "==> 2) Add vNEXT env"
backup_file state/live.env
cat >> state/live.env <<'EOF'

# ===== vNEXT =====
export VNEXT_TOP_K=60
export VNEXT_SIZE_MIN_MULT=0.70
export VNEXT_SIZE_MAX_MULT=1.20
export VNEXT_VOL_SCALE=50000
export VNEXT_TX_SCALE=250

# ===== price sanity =====
export SANITY_MAX_JUMP_MULT=6.0
EOF

echo
echo "==> DONE"
echo "Files created:"
echo " - src/vnext_guard.py"
echo " - core/price_sanity.py"
