...
# Si un token est "déjà dans le wallet" mais ultra dust, on l'ignore (et donc ça ne bloque pas les buys)
...
def _load_jsonl(path: str) -> List[Dict[str, Any]]:
    ...
    if skip:
        ...
        print(f"🧠 brain_v2: filtered skip_mints={before-len(items)} remaining={len(items)}")
    out=[]
...
def _write_jsonl(path: str, items) -> None:
    ...
    os.replace(tmp, path)
def _load_skip_mints(path: str) -> set:
    s=set()
    if not os.path.exists(path):
        return s
    with open(path, "r", encoding="utf-8") as f:
...
def score_market(o: Dict[str, Any]) -> float:
    # Idée: scorer sans exploser (log + clamp)
    ...
    # pondérations simples (stables)
    score = 0.0
    score += 3.0 * liq_s
    score += 2.0 * v1h_s
    score += 2.0 * t1h_s
    score += 1.0 * c1h_s
    score += -0.8 * mc_s  # pénalise les gros caps (option moonshot)

    return score

def score_flow(o: Dict[str, Any], skip_mints: set) -> float:
    # Pour l’instant: si mint dans skip => énorme malus (évite loops)
    m = str(o.get("mint") or "")
    if m in skip_mints:
        return -999.0
    return 0.0
...
def run_once() -> int:
    ...
    # Retire les tokens que Jupiter refuse (TOKEN_NOT_TRADABLE) si brain a déjà cette info
    try:
        out2=[]
        dropped=0
        for x in out:
            ...
            out2.append(x)
        if dropped:
            print(f"🧠 brain_v2: dropped TOKEN_NOT_TRADABLE={dropped}")
        out = out2
    ...
    print(f"🧠 brain_v2: in={len(items)} -> out={len(out)} file={READY_OUT}")
    if out:
        top=out[0]
        print("🧠 top1:", top.get("mint"), "score=", top.get("brain_score"))
    return 0
