...
def write_canonical(candidates: List[Dict[str, Any]], path: str = "") -> int:
    ...
    for cand in candidates:
        entry = {
            ...
            "liquidity_usd": float(cand.get("_liq_usd", cand.get("liquidity_usd", 0)) or 0),
            "market_cap_usd": float(cand.get("market_cap_usd", 0) or 0),
            "age_seconds": cand.get("age_seconds", 0),
            "jupiter_ok": True if cand.get("_jupiter_route") is True else (False if cand.get("_jupiter_route") is False else None),
