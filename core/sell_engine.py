...
class SellEngine:
    ...
    - utilise src/sell_exec.py pour exécuter les SELL via Jupiter
    ...
    def _sell_exec(self, mint: str, ui_amount: float, reason: str) -> str:
        ...
        if proc.returncode != 0:
            ...
            if 'computed amount <= 0' in stderr_txt:
                ...
                print('⚠️ SELL_SKIP_DUST mint=', mint, 'ui=', ui_amount, 'reason=', reason, '-> CLOSE', flush=True)
                try:
                    self.db.close_position(mint, close_reason='dust', close_ts=int(time.time()))
                except Exception:
                    pass
                return ''
    ...
    def run_once(self):
        ...
        try:
            ...
            if not getattr(self, '_cfg_logged', False):
                ...
                # print a few likely attributes if present
            ...
            print('⚠️ cfg log failed ' + str(_e), flush=True)
        now = _time.time()
    ...
    def _handle_one(self, pos, now: float):
        ...
        print("📈 PRICE mint=%s entry=%s price=%s pnl=%.2f%% tp1=%s tp2=%s hw=%s" %
        ...
        if pnl <= self.HARD_SL_PCT:
            ...
            # ANTI_DUST_CLOSE_V2
            if sell_qty <= 0:
                print('⚠️ SELL_SKIP_DUST mint=', mint, 'ui=', sell_qty, '-> CLOSE', flush=True)
                try:
                    self.db.close_position(mint, close_reason='dust', close_ts=now)
                except Exception:
                    pass
                return
