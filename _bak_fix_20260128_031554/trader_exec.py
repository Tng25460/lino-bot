...
if not DRY_RUN:
    try:
        txsig = _send_signed_b64(tx_b64, RPC_HTTP)
...
def _get_token_amount_for_sell(mint: str) -> Optional[int]:
    """
    On lit l'amount token à vendre depuis env:
...
def main() -> int:
    ...
    if sell_mode:
        ...
        if not sell_mint:
            ...
            print("❌ SELL_MODE=1 but SELL_MINT missing")
            return 0
        ...
        if token_amount is None:
            ...
            print("❌ SELL missing token amount env")
            return 0
        ...
        if not output_mint:
            ...
            print("⚠️ candidate sans mint/outputMint")
            return 0
...
def _send_signed_b64(tx_b64: str, rpc_http: str) -> str:
    ...
    r = requests.post(rpc_http, json=payload, timeout=35)
    try:
        j = r.json()
    except Exception:
        raise RuntimeError(f"RPC non-JSON status={r.status_code} body={r.text[:400]}")
...
def _send_signed_b64(tx_b64: str, rpc_http: str) -> str:
    ...
    tx.sign(kp)
    c = Client(rpc_http)
    resp = c.send_raw_transaction(tx.serialize(), opts={"skip_preflight": False, "max_retries": 3})
